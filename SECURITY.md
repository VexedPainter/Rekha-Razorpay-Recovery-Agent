...
what Rekha's Native Agent Gate and MCP proxy actually protect against, and
...
Rekha's safety path — `rekha/contracts`, `rekha/planner`, `rekha/policy`,
`rekha/approvals`, `rekha/executor`, `rekha/rewind`, `rekha/ledger`,
`rekha/intent`, `rekha/supervisor`, `rekha/hooks` — is deterministic: no
...
`rekha/cli` (adoption/DX: install, doctor, dashboard, export-pr) is lower
...
- Forging, corrupting, or silently reordering a ledger event (breaking the
  hash chain, spec §9) without detection by `rekha verify`.
...
- Reading or exfiltrating Rekha's own private storage (capability token,
  approvals database under `rekha_home()`) through a call the Native Agent
  Gate (`rekha/hooks/gate.py`) was supposed to have classified as safe.
...
Rekha's threat model does **not** claim to resist an arbitrary process
running as the same OS user as the agent (see
[`docs/security/threat-model.md`](docs/security/threat-model.md) and the
"TRUTH-010" invariant referenced in `rekha/hooks/gate.py` and
`rekha/hooks/claude_code_adapter.py`) — that's a stated limitation, not a
...
Rekha is pre-1.0 (`0.2.0a1`, unreleased to PyPI/npm as of this writing —
