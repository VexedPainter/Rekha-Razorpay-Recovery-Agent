...
The MCP proxy fences a session before a real (non-dry-run) `rekha rewind`
begins: `rekha/rewind/service.py::RewindService.rewind()` appends a
`session_fenced` ledger event, and `rekha/proxy/lifecycle.py`'s
...
The audit comparing the MCP proxy against `rekha/hooks/gate.py` (the
...
New CLI command: `rekha hooks fence <host_session_id> --host <host> --db
<db>`. Writes a `session_fenced` event to the ledger under the same
`hook-<host>-<host_session_id>` key `rekha/hooks/gate.py::ledger_session_id`
...
`rekha/supervisor/server.py::Supervisor._decide_pre` checks `is_fenced()`
...
(the CLI helper `rekha hooks fence`/`hooks approvals approve`/`reject`
...
`hooks run`/`hooks install`, but `rekha hooks fence` can legitimately be
...
- A human can now stop a hook session's future actions unconditionally,
  the same durable, cross-process way `rekha rewind` already stops an MCP
  session's.
- No automatic fencing: nothing in the Native Agent Gate fences a session
  ...
  fencing on some hook-side trigger (e.g. after `rekha hooks rewind`
...
- Does not address the remaining open R1 scope: Bash is still a static
  ...
  `initiated_by` ledger shapes in `rekha/policy/quota.py` and
  `rekha/policy/baseline.py`, which hook events don't share -- wiring
