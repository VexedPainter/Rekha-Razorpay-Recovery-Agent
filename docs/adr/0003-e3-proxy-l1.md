...
entrega que expone un proxy MCP real: Rekha como servidor MCP hacia el
...
- **La regla por defecto de §4.6, interpretada literalmente y sin
  ...
  []`, allow. Otherwise ⇒ Rekha MUST refuse to proxy the call with error
  ...
  `rekha/proxy/lifecycle.py::resolve()` implementa exactamente esto:
  ...
  (`rekha/proxy/server.py`) ni siquiera se lo pasa, solo pasa
...
- **Pipeline de spec §3 como clase `Lifecycle` con etapas intercambiables.**
  ...
  `Lifecycle` ni los call sites en `rekha/proxy/server.py`.
...
- **Transporte: stdio como mínimo exigido; HTTP streamable queda como
  ...
  exige "al menos stdio" y una abstracción limpia. `rekha/proxy/upstream.py`
  expone `connect_stdio()` como única función de conexión; el resto del
  proxy (`Lifecycle`, `RekhaProxyServer`) no conoce el transporte en
...
- **`rekha wrap` valida el contract set inmediatamente**, no en el primer
  `rekha run`: falla rápido si el fichero de contratos no compila o si
  `<server-dir>/server.py` no existe, antes de escribir `rekha.wrap.json`.
- **El entorno del subproceso upstream hereda `os.environ` completo, no solo
  ...
  Pero aquí quien lanza el proceso upstream es el propio operador de Rekha
  (vía `rekha run`), no el agente — así que `rekha/cli/main.py::run()` pasa
  `env=dict(os.environ)` explícitamente a `connect_stdio()`. Sin esto,
  `examples/fs-server`'s `REKHA_FS_ROOT` (y cualquier config equivalente de
...
- **`examples/fs-server` es un servidor MCP de verdad, no un mock.**
  ...
  sandboxed a un directorio (`REKHA_FS_ROOT` o un temp dir). Los nombres de
...
- El test de integración real por stdio (`tests/proxy/test_stdio_integration.py`)
  lanza **dos** subprocesos Python (uno para `rekha run`, otro para
  `examples/fs-server` que `rekha run` lanza a su vez) por caso de test.
...
- Para no depender de subprocesos en cada test del proxy, la mayoría de los
  tests de `rekha/proxy/server.py` usan el transporte en memoria del propio
  ...
  sin arrancar un proceso — cubre el lado agente↔Rekha del proxy con coste
...
- Código: `rekha/proxy/{lifecycle,server,upstream,config}.py`,
  `rekha/cli/main.py` (`wrap`, `run`), `examples/fs-server/server.py`.
