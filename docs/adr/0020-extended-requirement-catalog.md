...
`rekha/hooks/`, `rekha/supervisor/`, `rekha/db/models.py`, `rekha/cli/main.py`,
...
as if `docs/spec.md` (the Rekha Specification 0.1) defines them.
...
|---|---|---|
| ARCH-001 | The local supervisor: a persistent, authenticated per-install process (not a cold start per tool call, not a shared global daemon). | `rekha/supervisor/server.py` |
| ARCH-002 | Never an unauthenticated TCP port -- a Windows named pipe or POSIX Unix domain socket only. | `rekha/supervisor/server.py`, `rekha/supervisor/addressing.py` |
| ARCH-003 | Installation-scoped capability: the token/approvals data are scoped to one project-anchor install, not shared across unrelated projects. | `rekha/supervisor/auth.py`, `rekha/supervisor/addressing.py` |
| ARCH-004 | The capability token lives outside the project directory with restrictive permissions -- never solely an environment variable (an agent can set its own env vars). | `rekha/supervisor/auth.py`, `rekha/supervisor/addressing.py` |
| ARCH-006 | Duplicate hook event IDs MUST be idempotent *durably* -- across a supervisor restart, not just for one process's in-memory lifetime. | `rekha/supervisor/idempotency.py`, `rekha/db/models.py` |
...
| ARCH-008 | On-demand supervisor lifecycle (spawn if not already running); OS service-manager integration is explicitly not built yet (P1). | `rekha/supervisor/lifecycle.py`, `rekha/cli/main.py` |
...
| ID | Meaning | Primary site |
|---|---|---|
| FILE-001 | Native `Edit`/`Write`/`NotebookEdit` calls are captured (pre-edit snapshot) as a side effect of being allowed, so they can be rewound later. | `rekha/hooks/gate.py`, `rekha/hooks/file_snapshot.py` |
| FILE-004 | A rewind's conflict check: the file's *current* content must match the recorded post-edit hash before restoring -- something touched it since is refused, not clobbered. | `rekha/hooks/file_snapshot.py` |
| FILE-005 | A path that didn't exist before the edit is restored by *deleting* it, not by writing empty bytes. | `rekha/hooks/file_snapshot.py`, `rekha/db/models.py` |
| FILE-006 | A file exceeding the capture size cap downgrades reversibility and pauses for human approval instead of being silently allowed uncaptured. | `rekha/hooks/gate.py`, `rekha/hooks/file_snapshot.py` |
...
| ID | Meaning | Primary site |
|---|---|---|
| TRUTH-004 | A host integration may only claim `trust_tier: "T1"` (PROTECTED) after its pinned-version end-to-end bypass suite has actually been run and passed against a real installed binary -- never asserted ahead of that evidence. | `rekha/hooks/claude_code_adapter.py`, `tests/hooks/test_live_conformance.py` |
| TRUTH-010 | `T1` does not claim resistance to an arbitrary process running as the same OS user as the agent -- a stated scope limit, not an oversight. | `rekha/hooks/gate.py`, `rekha/hooks/claude_code_adapter.py`, [`docs/security/threat-model.md`](../security/threat-model.md) |

## Consequence

Code comments across `rekha/hooks/`, `rekha/supervisor/`, `rekha/db/models.py`,
`rekha/cli/main.py`, and their tests were corrected to stop attributing
