...
(`rekha/proxy/lifecycle.py`) por una cola de aprobaciones real
(`rekha/approvals/queue.py`) y las órdenes CLI `rekha approvals
list/approve/reject`.

## Decisiones

- **Máquina de estados (§7.1): `pending -> approved | rejected | expired`,
  transiciones unidireccionales.** `rekha/approvals/queue.py` codifica el
  ...
  `RekhaError("approval_expired")` específicamente cuando el origen es
...
- **Expiración perezosa, y gana el empate exacto (§7.1/§12 TOCTOU).** No hay
  ...
  Esto reutiliza el `Clock` inyectable de E4 (`rekha/clock.py`) — ningún
...
- **`pending_approval` es un resultado estructurado, no una excepción
  ...
  como valor de retorno normal (no lanza `RekhaError`) cuando el veredicto
  es `pause` y no hay un item ya `approved` para el `plan_id`.
  `rekha/proxy/server.py` envuelve cualquier retorno que no sea ya un
  ...
  se propagan como `RekhaError` (`approval_rejected`/`approval_expired`),
...
- **Binding por `plan_id`, no invalidación explícita (§12 approver
  ...
  por llamada (`rekha/planner/planner.py::_plan_id`): un reintento
...
- **Persistencia en SQLite (tabla `approvals`), no en memoria.** El plan
  ...
  `events`. Es necesaria de verdad: `rekha run` y `rekha approvals
  ...
  engine sobre el mismo fichero `rekha.db` cuando la CLI lo invoca por
  separado.
- **No-self-approval (§12) forzado arquitectónicamente, no solo por
  test.** `RekhaProxyServer._register_handlers()` solo registra
  ...
  *upstream* — nunca hay un tool `approvals.*` que un agente pueda
  ...
  `ApprovalQueue.approve`/`.reject` en todo `rekha/proxy/`. Esos verbos
  viven exclusivamente en `rekha/cli/main.py`. Tests:
  ...
  `.reject(`).
- **`rekha run` gana `--policy` (antes solo `rekha plan` lo tenía).**
  ...
  `default_policy()`). `RekhaProxyServer` acepta ahora un `PolicyDoc`
  opcional.
...
- Código: `rekha/approvals/queue.py`, `rekha/db/models.py` (`ApprovalRow`),
  `rekha/proxy/lifecycle.py` (`ApprovalStage`, `PendingApproval`,
  `ApprovalCheck`), `rekha/proxy/server.py`, `rekha/planner/planner.py`
  (`_plan_id`), `rekha/cli/main.py` (`approvals` subcommands, `run
  --policy`).
