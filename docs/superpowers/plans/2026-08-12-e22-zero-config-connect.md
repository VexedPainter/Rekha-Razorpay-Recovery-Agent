...
**Goal:** Let a user with Codex and/or Claude already installed connect the current directory to a Rekha-protected Filesystem MCP server with one `rekha connect` command and safely undo it with `rekha disconnect`.

**Architecture:** Keep `rekha.cli.main` thin and put immutable models, client adapters, and transaction orchestration in focused modules. Build the complete desired state and complete a real MCP preflight before writes, then use compare-and-swap snapshots for each official-CLI or renderer mutation; the project-owned manifest is the only authority for repair/disconnect.
...
- Create `rekha/bundled_packs.py`: resolve and validate bundled Filesystem pack metadata.
- Create `rekha/packs/filesystem/{pack.yaml,contracts.yaml}`: wheel-visible copy of the E20 pack.
- Create `rekha/cli/connection_models.py`: canonical project identity, snapshots, targets, manifest schema, and read-only inspection result.
- Create `rekha/cli/client_registration.py`: Codex/Claude command adapters and Claude Desktop fallback.
- Create `rekha/cli/connection.py`: preflight, transaction, rollback, verification, and disconnect orchestration.
...
- Modify `rekha/cli/main.py`: expose thin `connect`/`disconnect` commands and surface connection state through `doctor`/`repair`.
- Modify `rekha/cli/client_configs.py`: byte/hash CAS helpers and project-scoped hook merge/removal reuse.
...
**Files:**
- Create: `rekha/bundled_packs.py`
- Create: `rekha/packs/__init__.py`
- Create: `rekha/packs/filesystem/pack.yaml`
- Create: `rekha/packs/filesystem/contracts.yaml`
...
Expected: both Filesystem YAML files appear under `rekha/packs/filesystem/`.

- [ ] **Step 5: Commit**

```bash
git add rekha/bundled_packs.py rekha/packs tests/packs/test_bundled_filesystem_pack.py pyproject.toml
...
**Files:**
- Create: `rekha/cli/connection_models.py`
...
Keep serialization explicit and deterministic. Use atomic writes for `.rekha/connection.json` and never include secrets.
...
```bash
git add rekha/cli/connection_models.py tests/cli/test_connection_models.py
...
**Files:**
- Create: `rekha/cli/client_registration.py`
- Create: `tests/cli/test_client_registration.py`
- Modify: `rekha/cli/client_configs.py`
...
```bash
git add rekha/cli/client_registration.py rekha/cli/client_configs.py tests/cli/test_client_registration.py
...
- Create: `rekha/cli/connection.py`
- Modify: `rekha/proxy/upstream.py` only if a reusable timeout-safe preflight helper is required.

- [ ] **Step 1: Write failing runtime tests**

Assert `.rekha/rekha.wrap.json` uses the bundled contracts, `.rekha/rekha.db`, exact pinned upstream argv, and canonical current directory. For `sys.frozen`, assert the registered launch command is the absolute binary; otherwise assert the absolute interpreter plus `-m rekha.cli.main`.

- [ ] **Step 2: Write a failing real MCP-through-Rekha preflight**

Write the proposed runtime into the isolated project, start the exact launch argv that will be registered (`<python> -m rekha.cli.main run --config ...` or the frozen executable form), initialize that stdio process with the MCP SDK, and call `list_tools` through Rekha. Assert the advertised tools came from the pinned Filesystem upstream. Close Rekha, its upstream, and every stream. Skip only when `npx` is absent; CI must install Node and must not skip.
...
Perform all dependency, contract, directory, and upstream checks before client mutation. After runtime files exist, independently launch the exact proposed registered Rekha command and complete initialize/list-tools through the proxy before committing the connection. Bound both stages with timeouts and include the exact failed dependency/stage in the error.

- [ ] **Step 5: Prove directory confinement**

Through the generated Rekha proxy (never by connecting the test directly to the upstream), list/read inside `tmp_path` successfully and prove an outside path is unavailable or rejected.

- [ ] **Step 6: Commit**

```bash
git add rekha/cli/connection.py rekha/proxy/upstream.py tests/cli/test_connect_integration.py
...
**Files:**
- Modify: `rekha/cli/connection.py`
...
Mutate a target after Rekha's post-write snapshot but before rollback. Assert Rekha does not overwrite it, exits nonzero, names the file/registration, and persists `rollback_incomplete`.

- [ ] **Step 4: Write failing inspection and proxy-verification tests**

Require the public typed operation `inspect_connection(project_dir, name=None)`. It must be read-only, report every manifest/runtime/client/hook drift, identify `rollback_incomplete`, and distinguish a safe repair from a concurrent-edit conflict. The successful connect test must spawn the exact command read back from every fake client's recorded registration and complete MCP initialize/list-tools through Rekha before accepting `connected`.
...
Remove only when project/name/recorded post-write hash still match. Default writes `status: "disconnected"` plus UTC `disconnected_at`. Never remove `.rekha/rekha.db`.

- [ ] **Step 4: Run green and commit**

```bash
git add rekha/cli/connection.py tests/cli/test_connection.py
git commit -m "feat: disconnect only Rekha-managed entries"
```
...
Check `rekha connect --help`, no-arg defaults, `--name`, `--project`, `--client codex|claude|all`, `disconnect --purge-runtime`, concise success summary, collision error, no-client error, and rollback-conflict exit code. With a current-project connection manifest, `rekha doctor` must render `inspect_connection` without writing; `rekha repair` must safely call `connect` for repairable drift and refuse a `rollback_incomplete` concurrent-edit conflict with instructions to reconcile via `disconnect`.
...
```bash
git add rekha/cli/main.py tests/cli/test_connect_cli.py tests/cli/test_main.py
...
The driver accepts Rekha executable, fake client bin directory, isolated home/config roots, and project path; it runs connect twice, reads the exact launch argv recorded by the fake client, spawns that argv as an MCP stdio server, completes initialize/list-tools through Rekha, proves current-directory confinement through Rekha, disconnects, and verifies unrelated config survives. Connecting directly to the Filesystem upstream does not satisfy this smoke.
...
Install Node, build/install only the wheel, place fake Codex and Claude CLIs on PATH, set isolated homes, and execute `scripts/smoke_connect.py`. A missing bundled pack, wrong CLI syntax, wrong recorded Rekha launch command, failure to list tools through Rekha, or failed confinement must fail the job.

- [ ] **Step 4: Document the exact one-command experience**

Show `rekha connect`/`disconnect`, existing-client prerequisite, Filesystem-only current-directory default, pinned upstream, project-derived names, Claude hook scope, Codex MCP-only protection, and retained evidence DB.
...
Run: `ruff check . && mypy rekha && py -3.13 -m pytest tests/cli/test_connection_models.py tests/cli/test_client_registration.py tests/cli/test_connection.py tests/cli/test_connect_cli.py tests/cli/test_connect_integration.py tests/packs/test_bundled_filesystem_pack.py --no-cov`
...
Run: `py -3.13 -m pytest && py -3.13 -W error::ResourceWarning -m pytest -m "" --no-cov && py -3.13 scripts/traceability.py --check && py -3.13 -m conformance.cli run --target rekha --level 3`

Expected: all pass.

- [ ] **Step 3: Run isolated official syntax smoke**

Invoke installed `codex` and `claude` with isolated config/home environment variables and a temporary project; never read or mutate personal configuration. Verify `mcp add/get/remove` syntax and `rekha connect` end to end.
