...
`packs/filesystem/` and `packs/git/` are real, tested contract sets for
the actual official `@modelcontextprotocol/server-filesystem` (npm) and
`mcp-server-git` (PyPI) servers — not the illustrative `contracts/fs.yaml`
above, which targets belay's own example sandbox server. Use them the
same way, pointed at the real upstream:
...
belay wrap ~/projects/my-repo --contracts packs/git/contracts.yaml \
  --command mcp-server-git --arg -r --arg ~/projects/my-repo
```

Each `packs/<name>/pack.yaml` documents what upstream version it was
verified against and its known limitations (e.g. the filesystem server
has no delete tool at all, so creating a new file/directory has no undo
path through it; the git server returns plain text only, so most of its
mutations besides `git_add` are honestly `irreversible`, not silently
claimed otherwise). `trust_state: unverified` in both — that field is
about the signed-registry/release-policy chain spec §11.2 defines, which
doesn't exist yet, not a doubt about whether the pack works; see
[ADR 0019](docs/adr/0019-e20-verified-packs-scope.md) for the full scope
decision, including why GitHub/Odoo packs weren't attempted (they need
real API credentials/a real instance this environment doesn't have, and
this project doesn't ship contracts that were only checked against
mocks).
...
#### Deep doctor: `belay hooks doctor --deep` (E19.3)

The plain `belay hooks doctor` check above is config-only: it confirms
the hook entry is registered and matches what was installed, but not that
it would actually *work* right now. `--deep` verifies that for real —
the registered interpreter still exists on disk and can import belay, and
the supervisor for this install is genuinely reachable (spawning one on
demand if it isn't already running, exactly what a real hook invocation
would do). Slower, and not purely read-only (it may start a background
process as a side effect of checking reachability — said plainly, not
hidden), so it's opt-in rather than the default.

```bash
belay hooks doctor --deep
# -> claude-code: PreToolUse/PostToolUse hooks registered at .claude/settings.json -- unchanged since install, no backup
# -> claude-code:   deep check OK -- interpreter, belay import, and supervisor all reachable
```

#### Transactional repair: `belay repair` (E19.4)

Detects every belay-managed registration that's gone BROKEN (manifest
says it should be there, but the entry's been hand-removed since — the
same detection `belay doctor`/`belay hooks doctor` already report) across
every MCP client *and* hooks, and restores all of them in one command,
instead of requiring doctor → read the output → manually re-run
`init`/`hooks install` yourself for each one. It doesn't reimplement how
a registration gets written — each repair reuses `init`'s/`hooks
install`'s own atomic-write + backup + manifest machinery, so the same
safety guarantees apply. Anything that was never installed, or is already
healthy, is left completely alone.

```bash
belay repair --config belay.wrap.json
# -> this will repair:
#      claude-code: re-register 'belay'
#      claude-code hooks: re-register PreToolUse/PostToolUse
# -> repaired claude-code: re-registered 'belay' at .mcp.json
