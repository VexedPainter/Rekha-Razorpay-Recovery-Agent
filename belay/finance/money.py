...
3. **Evidence must be reproducible.** Ledger events are hashed
   (`belay/canonical.py`), and a float's shortest-repr can differ across
...
existing strict contract and policy documents (`belay/contracts/model.py`,
`belay/policy/model.py`) and inherits their validation and JSON
...
def _major_to_minor(major: str | int | Decimal | Any, currency: str) -> int:
    ...
    if not amount.is_finite():
        raise BelayError("money_invalid", {"reason": "amount must be finite", "value": str(major)})
