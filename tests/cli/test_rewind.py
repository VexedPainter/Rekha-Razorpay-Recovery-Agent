"""`rekha rewind` end-to-end (spec §10; plan.md E7 (d), the plan.md §10 demo).

Exercises the full CLI-driven flow against a real `rekha run` subprocess and
`examples/crm-mock`: an agent's `crm.bulk_delete` is paused (blast radius),
a human approves it via `rekha approvals approve`, a second ("oops") bulk
delete happens the same way, then `rekha rewind --dry-run` shows the honest
plan without touching anything, and `rekha rewind --by <name>` compensates
...
async def test_demo_scenario_pause_approve_oops_dry_run_and_rewind(tmp_path: Path) -> None:
    ...
    async with stdio_client(params) as (read, write), ClientSession(read, write) as session:
        ...
        # 2. `rekha approvals list` shows the real bound plan; human approves it.
        ...
        # Recover the session id rekha assigned (fixed per `rekha run` process).
        # `rekha verify` reads the whole ledger; find the session from the DB directly.
        ...
        for ev in all_events:
            assert ev.session_id == session_id  # one `rekha run` process => one session

    assert session_id is not None

    # 5. `rekha rewind --dry-run`: honest plan, nothing executed.
    ...
    # `rekha verify` after the dry run must still be clean -- nothing appended.
    verify_after_dry = runner.invoke(app, ["verify", str(db_path)])
    assert verify_after_dry.exit_code == 0

    # 6. `rekha rewind --by jairo`: real rewind, both bulk_delete steps undone
