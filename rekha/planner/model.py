...
from rekha.contracts.model import Contract
...
# transaction that is always rolled back -- see `rekha/planner/adapters/sql.py`)
...
# module) so `rekha/planner/model.py` stays a thin, dependency-light data
# module; `rekha/planner/adapters/sql.py` is the one place that touches
# SQLAlchemy.
...
class PlanningSession:
    ...
    already computed in `rekha/proxy/lifecycle.py` -- the planner does not
