...
- `sql_simulator` (plan-v2 E11) -- used when the contract declares a `sql`
  hint (`rekha/contracts/model.py::SqlHint`) and the caller supplies a
  `PlanningSession.sql_runner`. Runs the real statement in a transaction that
  is always rolled back (`rekha/planner/adapters/sql.py`) to get a real
...
from rekha.contracts.expressions import Scope
from rekha.contracts.expressions import evaluate as evaluate_expression
from rekha.contracts.expressions import parse as parse_expression
...
from rekha.planner.model import EffectEstimate, Plan, PlanningSession, SqlRunner
...
async def _sql_effects(
    ...
    Bind params are Rekha expressions (spec §4.3), evaluated against
