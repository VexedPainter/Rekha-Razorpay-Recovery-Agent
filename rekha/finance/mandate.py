...
- `max_cumulative` + `window` -- an aggregate ceiling over a rolling window.
  Enforced by `rekha/policy/cumulative.py`; declared here because it is the
...
from rekha.policy.quota import parse_window
...
def check_mandate(
    ...
    *,
    ...
    `rekha/policy/cumulative.py`, which has the ledger. This function stays a
...
def load_mandate(path: str | Path) -> MerchantMandate:
    ...
    `rekha/policy/model.py::load_policy`. Those errors describe an operator's
