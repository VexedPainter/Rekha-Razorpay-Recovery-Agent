"""`rekha dashboard`: a self-contained HTML snapshot of one ledger (spec §9).

Reads every event + approval item from a `rekha.db` and embeds them as JSON
...
command. Approve/reject actions are shown as the exact `rekha approvals`
...
<body>
<h1>Rekha dashboard (snapshot)</h1>
<div class="db-path">DATA_DB_PATH</div>
<div id="root"></div>
<script id="rekha-data" type="application/json">DATA_JSON</script>
<script>
const data = JSON.parse(document.getElementById("rekha-data").textContent);
