"""Typer entry point for the `rekha` CLI.

Subcommands (`wrap`, `run`, `plan`, `approvals`, `rewind`, `verify`) are
implemented incrementally in E3-E7; E0 only wired the app so `rekha
...
app = typer.Typer(
    name="rekha",
    help="Rekha: safe, reversible tool execution for AI agents.",
...
@app.callback()
def callback() -> None:
    """Rekha: safe, reversible tool execution for AI agents.
...
@app.command()
def verify(db: str = typer.Argument(..., help="Path to a Rekha SQLite ledger file.")) -> None:
...
def keygen(
    ...
    """
    from rekha.ledger.signing import SigningKey
...
def verify_export(
    ...
    key: str = typer.Option(
        ..., "--key", help="Path to an Ed25519 private signing key (PEM, `rekha keygen`)."
    ),
    db: str = typer.Option("rekha.db", "--db", help="Ledger SQLite file path."),
    ...
    from rekha.ledger.signing import SigningKey, sign_session
    from rekha.ledger.store import LedgerStore
...
def verify_evidence_cmd(
    file: str = typer.Argument(..., help="Signed evidence file (from `rekha verify-export`)."),
    pubkey: str = typer.Option(
        "",
        "--pubkey",
        help="Path to a trusted public key (hex text, `rekha keygen`'s .pub file). "
    ...
    No database, no network, no live Rekha installation required (plan-v2 E13).
    """
    from rekha.ledger.signing import SignedEvidence, verify_evidence
...
def dashboard(
    db: str = typer.Option("rekha.db", "--db", help="Ledger SQLite file path."),
    out: str = typer.Option(
        "rekha-dashboard.html", "--out", "-o", help="Where to write the dashboard HTML."
    ),
) -> None:
    ...
    re-running this command. Pending approvals show the exact `rekha
    ...
    """
    from rekha.cli.dashboard import render_dashboard
...
def verify_test(
    ...
    runner: str = typer.Option(
        ...
        "from the step's own `_rekha_test_ref`, never a string typed at verify-test time. "
        "This is the only path that can record mode='test' (VERIFIED in rekha causal).",
    ),
    cmd: str = typer.Option(
        "",
        "--cmd",
        help="Free-form command, for steps with no _rekha_test_ref. Recorded as "
    ...
    step's `_rekha_test_ref` from the ledger and builds the command from a
    fixed template (`rekha/ledger/test_evidence.py`'s `RUNNERS`) -- an
    ...
    pass/fail. Records ledger event `rekha:test_verified`
    (deliberately outside `rekha.ledger.model.EVENT_TYPES` -- adoption/DX,
    ...
    if runner:
        ...
        if not test_ref:
            typer.echo(f"error: step {step} has no _rekha_test_ref to verify", err=True)
            raise typer.Exit(code=1)
...
def replay(
    ...
    config: str = typer.Option("rekha.wrap.json", "--config", "-c", help="Wrap config path."),
    ...
    resume: str = typer.Option(
        "",
        "--resume",
        help="A replay session_id from a previous `rekha replay` run that paused for "
        "approval -- continue it from its next step instead of starting a new one "
        "(approve/reject the pause first via `rekha approvals`).",
    ),
) -> None:
    """Re-execute a real session against the live upstream, one step overridden.

    Unlike `rekha counterfactual` (plan-v2 E12, purely offline, never calls
    ...
    from rekha.contracts.loader import load_contract_set
    from rekha.ledger.store import LedgerStore
    from rekha.policy.model import default_policy, load_policy
            ...
            for step_seq in sorted(steps):
                ...
                if isinstance(result, dict) and result.get("status") == "pending_approval":
                    typer.echo(
                        ...
                        f"with `rekha approvals list/approve/reject --db {wrap_config.db}`, "
                        "replay stops here"
                    )
                    return
...
def draft_contracts_command(
    ...
    exactly for this: Rekha never treats an unreviewed contract as ground
    ...
    from rekha.contracts.draft import draft_contracts
    from rekha.proxy.config import UpstreamCommand
    from rekha.proxy.upstream import connect_stdio
...
def run(
    ...
    initiated_by: str = typer.Option(
        "",
        "--initiated-by",
        help="Identity (E14) that started this session. Overrides `rekha wrap`'s default; "
    ...
    """Start the Rekha MCP proxy over stdio (spec §3, Appendix C)."""
    import os
...
def plan_command(
    ...
    from rekha.contracts.loader import load_contract_set
    from rekha.planner.model import Plan, PlanningSession
    from rekha.planner.planner import Planner
...
def approvals_list(
    db: str = typer.Option("rekha.db", "--db", help="Ledger/approvals SQLite file path."),
    ...
    if triage:
        from rekha.approvals.triage import triage_queue
...
def rewind_command(
    ...
    intent: str = typer.Option(
        "",
        "--intent",
        help="Undo only steps tagged with this `_rekha_intent` label (adoption/DX, not "
    ...
    from rekha.proxy.config import WrapConfig
    from rekha.proxy.upstream import connect_stdio
    from rekha.rewind.service import RewindReport, RewindService
...
def counterfactual_command(
    ...
    so it works whether or not the session's `rekha run` is still live.
    """
    import json as jsonlib
...
from rekha.cli.recover import register as _register_recover  # noqa: E402

_register_recover(app)
from rekha.cli.webhooks import register as _register_webhooks  # noqa: E402

_register_webhooks(app)
from rekha.cli.settlement import register as _register_settlement  # noqa: E402

_register_settlement(app)
from rekha.cli.bench import register as _register_bench  # noqa: E402
