"""`rekha recover` -- run the AI recovery agent against a governed Razorpay surface.

Lives in `rekha/cli/` rather than in `recovery/` because of what it has to do:
...
def _describe_razorpay_action(
    ...
    `rekha/proxy/`, which must stay domain-agnostic -- and deliberately not a
...
def _record_forecasts(ledger: object, session_id: str, run: Any) -> None:
    ...
    importing `rekha.ledger` -- an agent able to write its own evidence could
...
def _report(run: Any, *, session_id: str, db: str) -> None:
    ...
    typer.echo(f"verify with        : rekha verify {db}")
