...
3. **calling** — the real tool call, deduplicated by `idempotency_key` when
   declared (`rekha/executor/idempotency.py`).
...
from rekha.contracts.expressions import Scope, evaluate, parse
...
from rekha.ledger.model import STEP_COMMITTED, STEP_FAILED
...
class SagaExecutor:
    ...
    async def compensate(
        ...
        Delegates to `rekha.rewind.service.compensate_one` (E7): one shared
        mini-step code path (journaled via `compensation_executed`/
        `compensation_failed`) for both this in-saga auto-unwind and the
        real `rekha rewind` command, replaying the frozen args verbatim --
        never re-evaluating the undo expression.
        """
        from rekha.rewind.service import compensate_one
