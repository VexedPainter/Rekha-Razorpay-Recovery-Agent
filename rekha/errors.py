"""Rekha error model (spec §11).

Defines the 17 error codes normative for 0.1, each with its `retryable`
flag. Full RekhaError exception class and error-construction helpers
...
FINANCE_ERROR_CODES: Final[Mapping[str, bool]] = {
    # `rekha/finance/money.py`: a malformed amount (a float, a bad decimal
    # string, more precision than the currency subdivides into).
    "money_invalid": False,
    # Arithmetic or ordering across two different currencies.
    "currency_mismatch": False,
    # `rekha/finance/mandate.py`: the merchant's grant of authority does not
    ...
    # `rekha/policy/cumulative.py`: this action would push windowed spend past
    ...
    # `rekha/settlement/verify.py`: money moved that does not reconcile
    # against what was authorized.
    "settlement_mismatch": False,
    # `rekha/razorpay/webhooks.py`: HMAC verification failed, so the payload
    # is not evidence of anything.
    "webhook_signature_invalid": False,
}

#: Every code `RekhaError` will accept: the normative 17 plus the financial
...
class RekhaError(Exception):
    ...
    def __init__(
        ...
        if code not in ERROR_CODES:
            raise ValueError(f"unknown Rekha error code: {code!r}")
        self.code = code
