...
Rekha reaches Razorpay's write APIs *only* through the official
`razorpay/razorpay-mcp-server` (MIT), launched as a wrapped stdio MCP
subprocess by `rekha wrap --command`. There is deliberately no HTTP client
for payments in this package: every money-moving call must traverse the
governed lifecycle in `rekha/proxy/lifecycle.py`, and a second, ungoverned
...
them usable as independent evidence in `rekha/settlement/`.
"""
