...
in-memory, because the CLI (`rekha approvals list/approve/reject`) runs as a
process separate from `rekha run` — the queue must outlive the proxy
process for the demo flow (plan.md §10) to work at all.

No-self-approval (spec §12) is enforced architecturally, not just by test:
this module exposes no MCP tool, is never imported by `rekha/proxy/server.py`
except to be *invoked by the operator's CLI process*, and `ApprovalStage`
(in `rekha/proxy/lifecycle.py`) only ever *reads* queue state on the agent's
...
from rekha.db.models import ApprovalRow, Base
...
class ApprovalNotConsumable(Exception):
    ...
    `approved` state (R1.6). Not a `RekhaError` -- see `consume`'s
    docstring for why."""
...
class ApprovalAlreadyConsumed(Exception):
    ...
    redelivery of the one that consumed it. Not a `RekhaError` -- see
...
class ApprovalItem:
    ...
    #: docstring (`rekha/db/models.py`) for what these mean and their
...
class ApprovalQueue:
    ...
    (the CLI's `rekha approvals` subcommands) -- never registered as an MCP
        ...
        with DBSession(self._engine) as db:
            ...
            if item.state == "expired" or self._clock.now() >= item.expires_at:
                if row.state == "pending":
                    row.state = "expired"
                    db.commit()
                raise RekhaError("approval_expired", {"approval_id": approval_id})
    ...
    def consume(
        ...
        *,
        ...
        - Not `approved` (`pending`/`rejected`/`expired`): raises
          ...
          ever. In practice unreachable from `rekha/hooks/gate.py` (it
        ...
        - `consumed_by_event_id is None` (first use): claims it for
          ...
          docstring in `rekha/db/models.py` for their current audit-only
        ...
        - `consumed_by_event_id == event_id` (the host redelivered the
          ...
          `rekha/supervisor/idempotency.py`'s own duplicate-event
        ...
        - `consumed_by_event_id` set to something ELSE: this approval was
          ...
          `ApprovalAlreadyConsumed` -- the caller (`rekha/hooks/gate.py`)
          must queue a fresh approval request rather than treat this as
          still covering the new instance.

        Not a `RekhaError`, deliberately: spec §11's `ERROR_CODES` is a
        fixed, normative registry of 17 codes for the MCP proxy's own
        protocol surface (`rekha/errors.py`) -- this is an internal
        Native Agent Gate signal `rekha/hooks/gate.py` catches directly,
