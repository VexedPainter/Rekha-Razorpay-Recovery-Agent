...
def _validate_sql_statement(statement: str) -> None:
    ...
    if not text:
        raise RekhaError("contract_invalid", {"reason": "sql.statement must not be empty"})
...
class SqlHint(_Strict):
    ...
    to a Rekha expression (spec §4.3) evaluated against `$args`/`$context` at
...
class AmountFrom(_Strict):
    ...
    carries the amount as two Rekha expressions (spec §4.3) instead, evaluated
