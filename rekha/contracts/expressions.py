"""The Rekha expression language (spec §4.3): parse() and evaluate().
...
class _Parser:
    ...
    def _fail(self, reason: str) -> None:
        raise RekhaError("expression_invalid", {"reason": reason, "text": self._text})
...
def parse(text: str) -> Expr:
    ...
    Raises `RekhaError(code="expression_invalid")` for malformed input,
    ...
    if not isinstance(text, str) or text.strip() == "":
        raise RekhaError("expression_invalid", {"reason": "empty expression"})
