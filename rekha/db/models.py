"""SQLAlchemy ORM models for Rekha's SQLite store (spec §9.1, §7).

`events` landed in E2 — it's all `LedgerStore` needs. `approvals` lands in
E5 alongside `rekha/approvals/queue.py` (spec §7): it must persist across
process invocations, since the CLI (`rekha approvals list/approve/reject`)
runs as a separate process from `rekha run`. `sessions`, `contract_sets`
...
class EventRow(Base):
    ...
    # event of the session (see rekha/ledger/model.py's Event docstring).
...
class ApprovalRow(Base):
    ...
    process from `rekha run`) can list/approve/reject items."""
    ...
    #: `rekha/approvals/queue.py::ApprovalQueue.consume`): the `event_id`
...
class IdempotencyRow(Base):
    ...
    paragraph 2), which is exactly what `rekha/executor/recovery.py`
...
class HookEventRow(Base):
    """One row per hook event the supervisor (`rekha/supervisor/`) has
    ...
    `rekha/supervisor/idempotency.py`.
    """
...
class FileSnapshotRow(Base):
    ...
    `rekha_home()/snapshots/<hash[:2]>/<hash>`, content-addressed by
