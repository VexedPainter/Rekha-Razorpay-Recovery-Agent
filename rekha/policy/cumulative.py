...
`rekha/policy/model.py::Cap` declared `per: session` from the beginning and
...
Design constraints inherited from `rekha/policy/quota.py`, which solved the same
...
def in_window(
    ...
    Boundary rule matches `rekha/policy/quota.py`: an action exactly `window` old
