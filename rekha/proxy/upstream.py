...
Rekha is an MCP client to the wrapped tool server: it launches it as a
...
async def connect_stdio(
    ...
    not, by itself, inherit the full parent environment) — Rekha passes its
    own `os.environ` through here so the upstream subprocess sees the same
    configuration (e.g. `examples/fs-server`'s `REKHA_FS_ROOT`) as `rekha run`.
    """
