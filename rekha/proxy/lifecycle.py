...
`rekha.approvals.queue.ApprovalQueue` and the agent gets back a structured
...
from rekha.approvals.queue import ApprovalAlreadyConsumed, ApprovalQueue
...
from rekha.finance.mandate import MerchantMandate, check_mandate
from rekha.finance.money import Money
from rekha.finance.money import total as sum_money
from rekha.ledger.model import STEP_FAILED
from rekha.ledger.redact import redact
from rekha.ledger.store import LedgerStore
from rekha.planner.model import NativeDryRunCaller, Plan, PlanningSession
from rekha.planner.planner import Planner
from rekha.policy.engine import PolicyEngine
from rekha.policy.explain import explain
from rekha.policy.model import PolicyDoc, PolicyResult, default_policy
...
def resolve(
    ...
    *,
    ...
    raise RekhaError("contract_missing", {"tool": tool})
...
class ApprovalStage:
    """Gates a `pause` verdict through `rekha.approvals.queue.ApprovalQueue` (spec §7).
    ...
    `reject` call sites -- those live only in `rekha/cli/main.py`'s
    ...
    def check(self, verdict: str, plan: Plan, session_id: str, step_seq: int) -> ApprovalCheck:
        ...
        if existing.state == "expired":
            raise RekhaError("approval_expired", {"approval_id": existing.approval_id})
        # still pending
        return ApprovalCheck(
...
class ExecuteStage:
    ...
    cycle of `rekha.executor.saga.SagaExecutor.run_step` -- journaled,
...
class Lifecycle:
    ...
    Injected rather than hard-coded because `rekha/proxy/` must stay
    ...
    paise and the method `args["method"]`. `rekha/razorpay/` supplies the
    ...
    `rekha/ledger/signing.py::SignedEvidence` can sign/verify it as part
    ...
    change), so `rekha/proxy/server.py` reads this side channel instead to
    ...
    def __post_init__(self) -> None:
        ...
        from rekha.canonical import canonical_hash
        ...
        if self.approval_stage is None:
            # Share the ledger's SQLite file (spec §7): the CLI's `rekha
    ...
    def _record_limit_refusal(
        ...
        raise RekhaError(code, {**detail, "mandate_hash": self.mandate.hash()})
    ...
    def start_session(self, initiated_by: str, on_behalf_of: str | None = None) -> None:
        ...
        (`rekha/canonical.py`), so which grant of authority governed this
        ...
        `rekha/ledger/signing.py::SignedEvidence` can sign and later
    ...
    async def govern_and_execute(
        ...
        *,
        ...
        from rekha.rewind.service import is_fenced

        if is_fenced(self.ledger, self.session_id):
            # spec §10.1: a fenced session is closed to new steps.
            raise RekhaError("session_fenced", {"session_id": self.session_id})
        ...
        # so `rekha rewind --intent`/causal-graph tooling can group steps by the
