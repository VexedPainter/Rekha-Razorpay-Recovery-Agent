"""`rekha rewind --intent/--keep`: selective rewind by declared subgoal (adoption/DX,
not spec-numbered -- builds on the real spec §10 `RewindService`, doesn't replace it).

An agent tags each tool call with a `_rekha_intent` label (see
`rekha/proxy/server.py`), recorded on that step's `plan_created` ledger
...
class IntentRewindError(Exception):
    """Adoption/DX error for `rekha rewind --intent` -- not part of spec §11's closed
    `RekhaError`/`ERROR_CODES` registry (that's the governed MCP tool-call error
