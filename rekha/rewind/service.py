...
- Fencing: a live session is fenced (closed to new steps) before a real
  (non-dry-run) rewind begins. `rekha.proxy.lifecycle.Lifecycle` checks
  ...
  (the CLI's `rekha rewind` runs separately from `rekha run`).
...
from rekha.ledger.model import STEP_COMMITTED, STEP_FAILED, STEP_INDETERMINATE, Event
...
_PLAN_TTL_SECONDS = 600  # mirrors rekha.planner.planner's default (spec §5.4)


def _as_dict(value: Any) -> dict[str, Any]:
    """Mirrors `rekha.executor.saga._as_dict`: unwrap a real `CallToolResult`'s
...
async def compensate_one(
    ...
    -- shared by `RewindService.rewind()` and `rekha.executor.saga.SagaExecutor
...
class RewindService:
    ...
    def __post_init__(self) -> None:
        if self.approvals is None:
            ...
            # `rekha approvals approve` runs as a separate CLI process.
    ...
    def _compensation_plan(
        ...
        # `rekha.planner.planner._plan_id` -- so an approval granted for one
