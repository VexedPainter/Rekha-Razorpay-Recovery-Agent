...
1. `_rekha_test_ref` is a bare string the agent itself supplies --
...
ran against -- `rekha causal`/`export-pr` label this case
...
def run_declared_test(
    ...
    be interpreted by. Recorded as `mode: "test"`, the only mode `rekha causal`/
