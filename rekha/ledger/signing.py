...
crypto) so a third party with no access to Rekha's database and no trust
...
This is additive over the existing hash chain (E2, `rekha/ledger/verify.py`)
-- it reuses `verify_chain`/`verify_coherence`, never recomputes the chain a
second way, and never changes the unsigned `rekha verify` path.
"""
...
from rekha.canonical import canonical_bytes
from rekha.ledger.model import Event, VerifyReport
...
def verify_signature(public_key_hex: str, data: bytes, signature_hex: str) -> bool:
    ...
    E13) and signed release bundle verification (`rekha release verify`,
...
class SignedEvidence(BaseModel):
    ...
    # `rekha/proxy/lifecycle.py::Lifecycle.__post_init__`) in effect for
...
def _policy_hash_from_events(events: list[Event]) -> str | None:
    ...
    `payload["policy_hash"]` (`rekha/proxy/lifecycle.py::Lifecycle.start_session`)
