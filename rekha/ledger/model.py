...
# event types, so the writers (rekha/executor/saga.py,
# rekha/executor/recovery.py) and the readers that classify by them
# (rekha/rewind/service.py, rekha/cli/causal.py) share one canonical
...
class Event(BaseModel):
    ...
    # session (an API key label, SSO claim, service account name -- Rekha
    ...
    # every event of the session -- `rekha.ledger.replay.SessionState`
