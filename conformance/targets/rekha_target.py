"""`RekhaConformanceTarget`: the reference adapter, wired straight to `rekha/`.

Reuses the real production wiring (`rekha.proxy.lifecycle.Lifecycle`,
...
from rekha.policy.model import PolicyDoc, default_policy, load_policy
from rekha.proxy.lifecycle import Lifecycle
from rekha.rewind.service import RewindService
...
class RekhaConformanceTarget:
    """Drives this repo's own implementation -- the "does Rekha conform to
    Rekha" sanity check, and the default for `rekha-conformance run --target rekha`.
    """
